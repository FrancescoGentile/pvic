...
class CustomisedDLE(DistributedLearningEngine):
        ...
        if self._rank == 0:
            ...
            print(
                ...
                "Time[Data/Iter.]: [{:.2f}s/{:.2f}s]".format(
                    ...
                    self.epochs,
                    str(
                        self._state.iteration - num_iter * (self._state.epoch - 1)
                    ).zfill(n_d),
                    num_iter,
                    running_loss,
                    t_data,
    ...
    def test_hico(self):
        ...
        for batch in tqdm(dataloader, disable=(self._world_size != 1)):
            ...
            if self._rank == 0:
                meter.append(
                    torch.cat(scores_ddp), torch.cat(preds_ddp), torch.cat(labels_ddp)
                )
    ...
    def cache_hico(self, dataloader, cache_dir="matlab"):
        ...
        for i, (image, target) in enumerate(tqdm(dataloader.dataset)):
            inputs = pocket.ops.relocate_to_cuda(
                [
                    image,
                ]
            )
