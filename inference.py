...
def draw_boxes(ax, boxes):
    ...
    for i, (a, b, c) in enumerate(zip(xy, h.tolist(), w.tolist())):
        ...
        ax.add_patch(patch)
        txt = plt.text(
            *a.tolist(), str(i + 1), fontsize=20, fontweight="semibold", color="w"
        )
...
def visualise_entire_image(image, output, attn, action=None, thresh=0.2):
    ...
    if action is not None:
        keep = torch.nonzero(
            torch.logical_and(scores >= thresh, pred == action)
        ).squeeze(1)
        ...
        for i in keep:
            ...
            attn_map = attn[0, :, ho_pair_idx].reshape(
                8, math.ceil(h / 32), math.ceil(w / 32)
            )
            attn_image = image_copy.copy()
            pocket.utils.draw_boxes(
                attn_image, torch.stack([bx_h[i], bx_o[i]]), width=4
            )
            if args.avg_attn:
                pocket.advis.heatmap(
                    attn_image,
                    attn_map.mean(0, keepdim=True),
                    save_path=f"pair_{i}_avg_attn.png",
                )
                plt.close()
            else:
                for j in range(8):
                    pocket.advis.heatmap(
                        attn_image,
                        attn_map[j : j + 1],
                        save_path=f"pair_{i}_attn_head_{j+1}.png",
                    )
                    plt.close()
...
def main(args):
    dataset = DataFactory(
        name=args.dataset, partition=args.partition, data_root=args.data_root
    )
    conversion = (
        dataset.dataset.object_to_verb
    ...
    if args.image_path is None:
        ...
        image = dataset.dataset.load_image(
            os.path.join(dataset.dataset._root, dataset.dataset.filename(args.index))
        )
    else:
    ...
    visualise_entire_image(
        image, output[0], attn_weights[0], args.action, args.action_score_thresh
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--image-path", default=None, type=str, help="Path to an image file."
    )
