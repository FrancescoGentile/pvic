...
class HICODETDataset(Dataset):
    ...
    def valid_interaction_tuples(self) -> Optional[List[List[int]]]:
        ...
        for couple in data:
            entity_id = self._entity_classes[couple["nname"]]
            interaction_id = self._interaction_classes[couple["vname"]]
    ...
    def __getitem__(self, idx: int) -> Tuple[Sample, Annotation]:
        ...
        for entity in sample["entities"]:
            ...
            entity_labels.append(self._entity_classes[entity["class"]])
        ...
        for idx, interaction in enumerate(sample["interactions"]):
            ...
            for inter_label in interaction["classes"]:
    ...
    def _get_samples(self) -> List[_SampleData]:
        ...
        data = [sample for sample in data if len(sample["entities"]) > 0]
        return data
